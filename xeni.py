    ...
    with tf.Session() as sess:
        ...
        with tf.device('/gpu:0'):
            def lstm_cell(k, prefix):
                return tf.nn.rnn_cell.BasicLSTMCell(hidden_size, name = prefix + str(k))
            ...
            generator_cell = tf.nn.rnn_cell.MultiRNNCell([lstm_cell(i, "generator_") for i in range(number_of_layers)])
